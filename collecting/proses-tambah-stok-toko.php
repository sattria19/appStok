...
$stokKurang = []; // untuk catat nama barang yang stoknya tidak cukup
...
foreach ($id_barang as $key => $barang_id) {
    ...
    if ($jumlah_tambah > 0) {
        ...
        if ($jumlah_tambah > $stok_gudang) {
            $stokKurang[] = "$nama_barang (maks $stok_gudang pcs)";
            continue;
        }
...
// Kalau ada stok yang tidak cukup, munculkan alert dan kembali
if (!empty($stokKurang)) {
    $pesan = "Stok gudang tidak mencukupi untuk:\n- " . implode("\n- ", $stokKurang);
